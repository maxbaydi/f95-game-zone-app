...
const GameDetailWindow = () => {
  ...
  useEffect(() => {
    ...
    const handleGameData = (event, fetchedGame) => {
      ...
      setFormData({
        ...
        release_date: fetchedGame.release_date
          ? new Date(parseInt(fetchedGame.release_date) * 1000)
              .toISOString()
              .split("T")[0]
          : "",
        status: fetchedGame.status || "",
        tags: fetchedGame.f95_tags
          ? fetchedGame.f95_tags.replace(/,/g, " , ")
          : "",
      ...
      window.electronAPI
        .getPreviews(fetchedGame.record_id)
        .then((urls) => {
          setPreviewUrls(urls || []);
        })
        .catch((err) => {
    ...
    window.electronAPI.onGameDetailsImportProgress(
  ...
  const handleDownloadPreviews = async () => {
    try {
      setImportProgress({
        text: "Starting previews download...",
        progress: 0,
        total: 1,
      });
  ...
  const handleSave = async () => {
    ...
    const updatedGame = {
      ...
      release_date: formData.release_date
        ? new Date(formData.release_date).getTime() / 1000
        : "",
  ...
  const handleFindGame = async () => {
    try {
      ...
      const results = await window.electronAPI.searchAtlas(
        formData.title,
        formData.developer,
      );
  ...
  const handleSelectGame = async (atlasId) => {
    try {
      ...
      window.electronAPI
        .getPreviews(updatedGame.record_id)
        .then((urls) => {
          setPreviewUrls(urls || []);
        })
        .catch((err) => {
              ...
              <div className="flex h-full">
                ...
                <div className="flex-grow p-4 space-y-2">
                  <div className="flex items-center">
                    ...
                    <input
                      name="game_version"
                      value={versionData.game_version}
                  ...
                  <div className="flex items-center">
                    ...
                    <input
                      name="game_path"
                      value={versionData.game_path}
                      onChange={handleVersionInputChange}
                      className="flex-grow bg-tertiary border border-border p-1 rounded"
                    />
                    <button
                      onClick={handleSetPath}
                      className="ml-2 px-2 py-1 bg-tertiary hover:bg-button_hover rounded"
                    >
                      Change
                    </button>
                  </div>
                  ...
                  <div className="flex items-center opacity-75">
                    ...
                    <input
                      name="version_size"
                      value={versionData.version_size}
                      disabled
                      className="flex-grow bg-tertiary border border-border p-1 rounded cursor-not-allowed"
                    />
                  </div>
                    ...
                    <div className="flex flex-col flex-grow">
                      ...
                      <div className="flex space-x-2 mt-2">
                        ...
                        <button
                          onClick={async () => {
                            try {
                              await window.electronAPI.deleteBanner(
                                game.record_id,
                              );
                              console.log(
                    ...
                    {Array.isArray(validPreviewUrls) &&
                      validPreviewUrls.length > 0 && (
                        <button
                          onClick={async () => {
                            try {
                              await window.electronAPI.deletePreviews(
                                game.record_id,
                              );
                              console.log(
                  ...
                  {searchResults.map((result, index) => (
                    ...
                    >
                      ...
                      <div className="text-sm text-gray-400">
                        Atlas ID: {result.atlas_id} | F95 ID:{" "}
                        {result.f95_id || "N/A"} | Creator:{" "}
                        {result.creator || "N/A"}
                      </div>
                    </li>
                  ))}
